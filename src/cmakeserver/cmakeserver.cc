...
private:
  ...
  std::unique_ptr<std::thread> m_worker; ///< Thread that for the cmakeserver handling
  /// Thread function
  void workerFunction();
...
void CMakeServer::workerFunction() {
  ...
  while (m_isRunning) {
    ...
    for (;;) {
      ...
      if (!foundStart) {
        if (readTemp.size() <= 1 + size(CMAKE_SERVER_HEADER_START) +
      ...
      if (std::string(document["type"].GetString()).compare("error") == 0) {
        ...
        // onError(document["errorMessage"].GetString());
      ...
      // "hello"}
      // ]== "CMake Server" ==]
      else if (std::string(document["type"].GetString()).compare("hello") ==
               0) {
        LOG_S(INFO) << "[CMakeServer] Received hello-Message";
        const rapidjson::Value &versions =
            document["supportedProtocolVersions"];
        ...
        for (auto &line : versions.GetArray()) {
          ...
          int const minor =
              line.HasMember("minor") == true ? line["minor"].GetInt() : 0;
        ...
        if (!found) {
          ...
          // onError("Version is not supported!");
          m_files = extractCacheCMakeServer(m_pathCache);
          m_isExtracted = true;
          m_cond.notify_all();
          m_isRunning = false; // Leave Thread!
      ...
      // ]== "CMake Server" ==]
      else if (std::string(document["type"].GetString()).compare("reply") ==
               0) {
        assert(document.HasMember("inReplyTo"));
        if (std::string(document["inReplyTo"].GetString())
                .compare("handshake") == 0) {
          // Send configure
          LOG_S(INFO) << "[CMakeServer] --> configuring ...";
          m_terminal->write_blocking(CMAKE_SERVER_COMMAND_CONFIGURE);
        } else if (std::string(document["inReplyTo"].GetString())
                       .compare("configure") == 0) {
          ...
          {
            std::fstream temp(m_pathCache,
                              std::ofstream::out | std::ofstream::trunc);
            if (temp) {
              temp << message.data();
              LOG_S(INFO) << "[CMakeServer] Created Cache: " << m_pathCache;
            } else
              LOG_S(ERROR) << "[CMakeServer] Couldn't create Cache! "
                           << m_pathCache;
          }
          ...
          LOG_S(ERROR) << "[CMakeServer] Unsupported Reply, "
                       << document["inReplyTo"].GetString();
        }
      }
      ...
      //]== "CMake Server" ==]
      else if (std::string(document["type"].GetString()).compare("message") ==
               0) {
        if (document.HasMember("title") &&
            std::string(document["title"].GetString()).compare("Error") == 0) {
      ...
      else if (std::string(document["type"].GetString()).compare("progress") ==
               0) {
      ...
      // Signal Message
      else if (std::string(document["type"].GetString()).compare("signal") ==
               0) {
        if (document.HasMember("name") &&
            std::string(document["name"].GetString()).compare("dirty") == 0) {
          if (!m_rebuilding) {
            m_rebuilding = true;
...
createCMakeServer(std::string const pathCache,
                  std::string const &buildDirectory,
                  std::string const &sourceDirectory,
                  std::unique_ptr<ICMakeServerTerminal> terminal){
  return std::make_unique<CMakeServer>(pathCache, buildDirectory,
                                       sourceDirectory, std::move(terminal));
}
...
extract(rapidjson::Document const &document) {
  ...
  std::vector<std::string> endings{".c", ".cc", ".cpp", ".cxx"};
      ...
      for (auto &projects : config["projects"].GetArray()) {
        ...
        if (projects.HasMember("targets") && projects["targets"].IsArray()) {
          for (auto &target : projects["targets"].GetArray()) {
            ...
            // Exclude targets
            if (std::string(target["type"].GetString()).compare("UTILITY") !=
                    0 &&
                target.HasMember("sourceDirectory") &&
                target.HasMember("fullName")) {

              std::string const sourceDirectory =
                  std::string(target["sourceDirectory"].GetString());
              // tempTarget.Filename =
              // std::string(target["fullName"].GetString());

              if (target.HasMember("fileGroups") &&
                  target["fileGroups"].IsArray()) {
                for (auto &fileGroup : target["fileGroups"].GetArray()) {
                  ...
                  if (fileGroup.HasMember("sources") &&
                    ...
                    if (fileGroup.HasMember("compileFlags")) {
                      std::stringstream ss(
                          fileGroup["compileFlags"].GetString());
                    ...
                    if (fileGroup.HasMember("defines") &&
                        fileGroup["defines"].IsArray()) {
                    ...
                    if (fileGroup.HasMember("includePath") &&
                      ...
                      for (auto &item : fileGroup["includePath"].GetArray()) {
                        auto tempPath = std::string(item["path"].GetString());
